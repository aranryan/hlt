...
manual_cw_oe_msa <- read.csv("input_data/manual_cw_oegc_msa.csv", head=TRUE, colClasses=colc)


# reads in an OE Asian Cities csv file using Hadley's readxl and saves as Rdata
temp_a <- read_excel("input_data/Global Cities - HLT - China_2015_09_16.xlsx", sheet=2)
...
temp_c <- temp_b %>%
  ...
  rename(oegc_location = location) %>%
  rename(oegc_loc_code = location_code) %>%
...
manual_cw_oe_msa <- manual_cw_oe_msa %>%
  select(-oegc_location)
temp_d <- temp_c %>%
  left_join(., manual_cw_oe_msa, by=c("oegc_loc_code" = "oegc_loc_code")) %>%
  mutate(area_sh = ifelse(oegc_location == "China", "chn", area_sh)) %>%
  select(date, area_sh, oe_code, value)

# changes some oe_codes to be consistent with the NA cities codes
temp_e <- temp_d %>%
  mutate(oe_code = gsub("GDPTOTLCC", "gdp", oe_code))
...
# rename the China GDP series so that I can remember it came from Global cities
temp_f <- temp_f %>%
  data.frame(date=time(.), .) %>%
  rename(gdpgc_chn = gdp_chn) %>%
  read.zoo %>%
  xts()

# spread to tidy format
out_gcities_a <- temp_f %>%
  data.frame(date=time(.), .) %>%
...
colnames(temp_h)
autoplot(diff(log(temp_h$gdpgc_chn)))
autoplot(diff(log(temp_h$gdp_beijichn)))
autoplot(diff(log(temp_h$gdp_chgduchn)))
autoplot(diff(log(temp_h$gdp_gngzhchn)))
autoplot(diff(log(temp_h$gdp_shangchn)))
...
out_gcities_q <- gcities_q %>%
  mutate(gdp = gdp/4/1000) %>%
  mutate(gdpgc = gdpgc/4/1000)
...
# saves Rdata version
save(out_gcities_a, file=paste("output_data/out_gcities_a.Rdata", sep=""))
save(out_gcities_q, file=paste("output_data/out_gcities_q.Rdata", sep=""))
